...
本项目基于 GPL-3.0 开源协议分发，源代码与原始设计基础来自原作者的公开仓库：

- 原始仓库：<https://github.com/XxHuberrr/Mineradio>
- 该仓库主页中已声明原作者永久停止更新；因此，当前仓库中的发布内容应视为第三方维护/修复分支，而非原作者官方更新版本。

请用户在下载、使用或分发时确认：

- 本项目不代表原作者在其官方仓库中发布了新版本；
- 当前维护分支版本是 `2.2.4`，用于区分本仓库编译产物；
- 原作者官方渠道中保留的版本号仍为 `2.2.0`，用于标识原始版本，不代表当前维护分支的官方分发；
- 本项目仅为基于 GPL-3.0 许可的源码分支/维护版本；
- 任何修改、重分发或打包行为必须保留原始版权声明、许可证与源码可获取说明；
- 若需要确认原作者的官方状态，请以原始仓库主页为准，不应将本维护版本误认为原作者官方发布。

如需以原作者仓库为准，请务必优先查看原始仓库主页与 Release 页面，确保不会混淆官方版本与第三方分支版本。
...
安装时只需要下载并运行 `Mineradio-2.2.4-Setup.exe`。不要把 `.blockmap`、`latest.yml` 或 `win-unpacked` 当成正式安装包。

2.1.0 用户如果未看到更新提醒，请从托盘彻底退出后重新打开软件，切回普通窗口，等待约 30 秒后查看右上角更新箭头。旧版不会自动弹出公告；也可以直接使用上面的新网盘链接下载安装包。

## 下载或安装被拦截怎么办

小众 Electron 桌面软件、未签名安装包有时会被浏览器、Windows Defender 或 SmartScreen 提示风险。请先确认安装包来自本次公告的下载入口，文件名是 `Mineradio-2.2.4-Setup.exe`。
...
状态：Mineradio 2.2.4 维护分支版。

> 安全提示：`v1.0.10` 及更早旧安装包不再建议继续安装或传播。请使用本次公告提供的 `Mineradio-2.2.4-Setup.exe`。
...
维护分支正式分发以 `Mineradio-2.2.4-Setup.exe` 为准，不建议直接使用 `win-unpacked` 目录。安装包会创建桌面快捷方式。

已经安装过旧版本的用户可直接运行 `Mineradio-2.2.4-Setup.exe` 完成更新。软件内更新入口只会打开浏览器下载页，不会在客户端内下载或应用补丁。
