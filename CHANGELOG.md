...
- 修复本地音乐播放时的误报问题：点击最近播放和音乐库继续播放时，已补上本地库恢复与有效性检查，避免误提示“本地文件已失效，请重新导入”。
- 修复已删除本地文件仍残留于本地音乐列表的问题，持久化库加载时会自动剔除失效记录。
- 优化搜索体验：本地音乐命中会在搜索结果中置顶显示，网络搜索结果仍保留在后方，便于快速找到本地文件。
- 进一步增强本地音乐恢复链路，确保本地播放状态、继续播放和重启后恢复都能从持久化库中正确重建 `localUrl`。
- 修复本地音乐因编码问题导致的乱码问题：本地文件的内嵌歌词、同名 LRC、GB18030 及 UTF-8 修正后的元数据会按同一套规则统一解析，避免标题、歌手、专辑和歌词因编码异常导致的乱码、缺失或匹配失败。
- 同步更新版本信息与发布说明，当前维护分支源码版本已提升至 `2.2.4`。原作者官方渠道仍保留 `2.2.0` 版本标识，不代表当前维护分支官方发布。
